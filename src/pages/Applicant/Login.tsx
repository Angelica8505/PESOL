    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#f4f6fb] dark:bg-slate-950 font-sans relative">
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-display font-bold text-slate-900 dark:text-white mb-2">{t('welcomeBack')}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">{t('signInApplicantDesc')}</p>
        </div>
        ...
        >
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white text-sm transition-all"
            ...
            >
              {loading ? (
                ...
                <>
                  {t('signInApplicant')} <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                </>
              )}
            </button>
          </form>
          ...
          <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 text-center text-xs space-y-3">
            <p className="text-slate-500 dark:text-slate-400">{t('notApplicant')}</p>
            <div className="flex justify-center gap-4">
              <Link to="/employer-portal" className="font-bold text-blue-600 hover:underline">
                {t('employerLoginLink')}
              </Link>
              <span className="text-slate-300 dark:text-slate-700">|</span>
              <Link to="/peso-portal" className="font-bold text-emerald-600 hover:underline">
                {t('pesoAdminLink')}
              </Link>
            </div>
          </div>
