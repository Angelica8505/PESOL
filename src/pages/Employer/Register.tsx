...
export default function EmployerRegister() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('Company account created successfully! Start posting jobs.', 'success');
      navigate('/employer');
    } catch (err: unknown) {
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs font-bold mb-4">
            New Employer Application Focus
          </div>
          <h1 className="text-3xl font-display font-bold text-slate-900 dark:text-white mb-2">Register Company</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Join our local employer ecosystem to source the best verified talent</p>
        </div>
        ...
        >
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider ml-1">Company Name</label>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider ml-1">Company Business Email</label>
              <div className="relative">
            ...
            >
              {loading ? (
                ...
                <>Apply as Employer <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" /></>
              )}
            </button>
          </form>
          ...
          <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 text-center text-xs space-y-3">
            ...
            <div className="flex justify-center gap-4">
              <Link to="/register" className="font-bold text-blue-600 hover:underline">Applicant Sign Up</Link>
            </div>
          </div>
        </motion.div>

        <p className="text-center mt-6 text-sm text-slate-500 dark:text-slate-400">
          Already have an account? <Link to="/employer/login" className="font-bold text-blue-600 hover:underline">Sign In</Link>
        </p>
      </div>
    </div>
  );
